    ...
    if (!selection) {
      ...
      TemplatesService.insertTemplate(req.app.get("db"), newTemplate)
        .then(template => {
          ...
          TemplatesService.updateDoc(req.app.get("db"), doc_id, updateDoc)
            .then(doc => {
              res
                .status(201)
...
templatesRouter
  .route("/:_id")
  .all((req, res, next) => {
    TemplatesService.getById(req.app.get("db"), req.params._id)
      .then(template => {
        if (!template) {
  ...
  .delete(requireAuth, (req, res, next) => {
    ...
    TemplatesService.deleteTemplate(req.app.get("db"), req.params._id)
